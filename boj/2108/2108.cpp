...
void solution()
{
    ...
    for (const auto& i : map) {
        if (i.second != max_mode)
            continue;
        n_mode.push_back(i.first);
    }
    sort(n_mode.begin(), n_mode.end());

    int avg = round(accumulate(numbers.begin(), numbers.end(), 0) / (n * 1.0));
    int median = numbers[numbers.size() > 1 ? (numbers.size() / 2) : 0];
    int mode = n_mode.size() > 1 ? n_mode[1] : n_mode.front();
