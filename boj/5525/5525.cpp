...
void solution()
{
    // hash slide (rabin karf)
    // n <= 1'000'000
    // I + OI

    // const long long n_max = 1'000'000;
    const long long b = 31;
    const long long mod = C_MAX;
    int n, m;
    string s;

    cin >> n >> m >> s;
    int window_size = n * 2 + 1;
    ...
    for (int i = 0; i < window_size; ++i) {
        pattern_hash *= b;
        pattern_hash %= mod;
        pattern_hash += (pattern[i] - 'A' + 1) % mod;
    }

    for (int i = 0; i < window_size; ++i) {
        niddle_hash *= b;
        niddle_hash %= mod;
        niddle_hash += (s[i] - 'A' + 1) % mod;
    }

    int answer = 0;

    for (int i = 0; i <= m - window_size; ++i) {
        if (pattern_hash == niddle_hash) {
            // && pattern == s.substr(i, window_size)
            ++answer;
        }
        niddle_hash += mod - (s[i] - 'A' + 1) * e % mod;
        niddle_hash %= mod;
        niddle_hash *= b;
        niddle_hash %= mod;

        if (i < m - window_size) {
            niddle_hash += (s[i + window_size] - 'A' + 1);
            niddle_hash %= mod;
        }
    }
    cout << answer;
