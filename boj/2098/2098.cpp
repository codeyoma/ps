...
void solution()
{
    ...
    int max_set = 1 << n;
    vector<vector<int>> dp(n, vector<int>(max_set, C_MAX));

    dp[0][1 << 0] = 0;

    for (int mask = 0; mask < max_set; ++mask) {
        for (int current = 0; current < n; ++current) {
            if (dp[current][mask] == C_MAX)
                continue;
            ...
            for (int next = 0; next < n; ++next) {
                ...
                if (next_mask == max_set - 1) {
                    if (cost[next][0] == 0)
                        continue;

                    dp[next][next_mask] = min(
                        dp[next][next_mask],
                        dp[current][mask] + cost[current][next] + cost[next][0]);

                } else {
                    dp[next][next_mask] = min(
                        dp[next][next_mask],
                        dp[current][mask] + cost[current][next]);
                }
            }
        }
    }
