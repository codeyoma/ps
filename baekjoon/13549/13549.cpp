...
void solution()
{
    ...
    while (!q.empty()) {
        ...
        if (tele <= limit && 0 <= tele) {
            if (dist[pos] < dist[tele]) {
                dist[tele] = dist[pos];
                q.push_front(tele);
            }
        }

        for (int new_p : { pos - 1, pos + 1 }) {
            if (0 <= new_p && new_p <= limit && dist[pos] + 1 < dist[new_p]) {
                dist[new_p] = dist[pos] + 1;
                q.push_back(new_p);
            }
        }
    }
