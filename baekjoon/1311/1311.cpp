...
#include <vector>
int _count_bit(const int& size, const int& mask)
{
    int count = 0;
...
void solution()
{
    ...
    int max_set = (1 << n);
    vector<vector<int>> dp(n + 1, vector<int>(max_set, C_MAX));
    dp[0][0] = 0;

    // for (int i = 0; i < n; ++i){
    for (int mask = 0; mask < max_set; ++mask) {
        // int i = _count_bit(n, mask);
        ...
        for (int next = 0; next < n; ++next) {
            if (((1 << next) & mask))
                continue;
            int next_job = mask | (1 << next);
            dp[i + 1][next_job] = min(
                dp[i + 1][next_job],
                dp[i][mask] + cost[i][next]);
        }
    }
    // }
