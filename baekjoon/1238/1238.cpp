...
void solution(){
    ...
    vector<vector<pair<int, int> > > r_graph(n, vector<pair<int, int> >(0));
    ...
    for (size_t i = 0; i < m; i++){
        ...
        r_graph[v - 1].push_back(make_pair((u - 1),w));
    }
...

    {
        ...
        while (!pq.empty()) {
            ...
            if (n.first != dist[n.second])
                continue;

            for (const auto& it : graph[n.second]){
...

    {
        ...
        r_dist[s] = 0;
        pq.push(make_pair(r_dist[s], s));

        while (!pq.empty()) {
            pair<int, int> n = pq.top();
            pq.pop();

            if (n.first != r_dist[n.second])
                continue;

            for (const auto& it : r_graph[n.second]){
                if (r_dist[it.first] > r_dist[n.second] + it.second){
                    r_dist[it.first] = r_dist[n.second] + it.second;
                    pq.push(make_pair(r_dist[it.first], it.first));
                }
            }
        }
    }
    ...
    for (size_t i = 0; i < n; i++){
        max = std::max(max, dist[i] + r_dist[i]);
    }
