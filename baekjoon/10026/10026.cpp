...
void solution()
{
    ...
    vector<vector<char>> normal(n, vector<char>(n));
    vector<vector<char>> color_blind(n, vector<char>(n));
    ...
    auto bfs = [&](vector<vector<char>>& paint) {
        int count = 0;
    ...
    cout << bfs(normal) << " " << bfs(color_blind);
}
